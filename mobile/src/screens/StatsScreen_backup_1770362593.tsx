...
export default function StatsScreen({ navigation }: StatsScreenProps) {
  ...
  // Calculate wellbeing score (0-100) - Pure math, no colors
  ...
  // Generate insights based on usage patterns - More data-driven
  ...
  // Load usage data from native module - Fixed to prevent blank screen
  const loadUsageData = async (isRefresh = false) => {
    try {
      ...
      if (stats && stats.length > 0) {
        ...
        try {
          // @ts-ignore - Method may not exist yet in native module
          ...
          if (hourlyBreakdown && Array.isArray(hourlyBreakdown)) {
            ...
            if (peak) {
              const hourStr = peak.hour === 0 ? '12am' : 
                             peak.hour < 12 ? `${peak.hour}am` :
                             peak.hour === 12 ? '12pm' : `${peak.hour - 12}pm`;
              setPeakHour(hourStr);
            }
          }
        } catch (err) {
